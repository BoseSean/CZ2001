...
public class HashTestApp {
	...
	public static void hashingDemo(int numOfValues, int search) {
		...
		HashTable inventory = new HashTable(numOfValues*2,"MOD_TABLE_SIZE_HASH");

		System.out.println("Searching for key "+ search);

		for (int i = 0; i < numOfValues; i++) {
			// dummy string
			inventory.add(i, "Shelf No.: " + i);
		}
	...
	public static void startTest(ArrayList<Integer> listOfNumbers, int numOfData, int reps, boolean success) {
		...
		for (int rep = 0; rep < reps; rep++) {
			...
			HashTable clientHashMap = new HashTable(1000, "MOD_TABLE_SIZE_HASH");
			for (int i = 0; i < numOfData; i++) {
				// dummy string
				clientHashMap.add(listOfNumbers.get(i), "Client #" + listOfNumbers.get(i));
			}
			end = System.nanoTime();

			putTime += end - start;
			putProbeCount = clientHashMap.getAddProbeCount();
